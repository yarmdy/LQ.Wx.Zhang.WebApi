...
namespace LQ.Wx.Zhang.Common
{
    ...
    public class JHttpH
    {
        ...
        private string Request(enumMethod method,string url, object? headers, object? bodys)
        {
            try
            {
                ...
                ModelHelperHttp.CopyFrom(header,headers!);
        ...
        {
            using (var zipStream = new BrotliStream(compressStream, CompressionMode.Decompress))
            {
                using (var resultStream = new MemoryStream())
                {
                    zipStream.CopyTo(resultStream);
    ...
    internal static class ModelHelperHttp
    {
